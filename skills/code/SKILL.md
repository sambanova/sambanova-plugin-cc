...
Run: `bash scripts/code.sh <tool> <model> <prompt> <cwd> [files...]` relative to this skill's directory.

### Arguments

| Arg | Required | Description |
|---|---|---|
| `tool` | yes | Coding tool to use. Currently only `continue` is supported. |
...
| `files` | no | Optional list of files to grant access to (reserved for future tools). |
